...
const OptimizationResults = ({ results, projectRequirements }) => {
  ...
  const validateBESSConfig = (bestConfig, requirements) => {
    ...
    if (bestConfig && requirements) {
      // Power validation (check overbuild)
      ...
      // Energy validation (check overbuild)
  ...
  useEffect(() => {
    ...
    try {
      // Parse BESS Assistant JSON response
      let jsonData;
      ...
      if (typeof jsonData === 'string') {
        console.log('Found nested JSON string, parsing again...');
      ...
      if (!jsonData || !jsonData.status) {
        throw new Error('Invalid BESS Assistant JSON schema - missing status field');
      }
